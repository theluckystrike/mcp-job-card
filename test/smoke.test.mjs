...
test.skip("the status flow moves one step at a time, stamped, and closes the card to entries", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
...
test.skip("print renders markdown and self-contained HTML, both with the signature line", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
...
test.skip("the summary answers a day and a week, per worker and per currency", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
...
test.skip("guards: future dates, bad hours, ambiguous refs, and delete keeps worked cards", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
...
test.skip("free tier: the eleventh active card is refused, archiving frees the slot, Pro lifts the cap", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
